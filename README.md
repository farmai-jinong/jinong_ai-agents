...
- 에이전트는 farmos 에 쓰지 않는다. 영농일지는 앱의 5개 블록(주요 농작업·기타 기록사항·병해충·방제이력·사진)에 맞춰 markdown + `PutDiaryDTO` 형태 prefill 로 돌려주고, 농가가 앱에서 확인 후 저장한다. 마크다운은 첫 줄 H1 `# 영농일지 — 작물 (날짜)` 로 시작하고(백엔드 H1/H2 파싱 계약), `##` 섹션 집합은 항상 고정이며(빈 섹션은 `언급 없음` 한 줄), H1 아래 `> 📝 통화 요약 / > 💬 격려` 인용 블록은 섹션이 아니라 읽는 사람용 머리말이라 prefill 에 들어가지 않는다.
