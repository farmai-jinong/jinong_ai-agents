...
- farmworks: 농가가 **수행했거나 수행 예정인 농작업**(관수, 적심, 적엽, 측지제거, 유인, 수확, 적과, 시비/추비, 약제살포, 환기, 정식, 수분 등). `name` 은 짧은 표준 명사형(예: "관수", "적엽", "약제살포").
  - when: today(오늘·통화 당일에 함) / ongoing(이전에 시작해 통화일에도 계속하고 있다고 명시적으로 말한 반복·상시 작업; date_hint 에 "지난주부터" 등 원문) / past(그 이전에 함; date_hint 에 "지난주 화요일", "3일 전" 등 원문) / planned(할 예정) / unknown(시점 불명).
...
- products: 농약·비료·종자·농자재 등 투입 제품. `name` 은 원문 상표/제품명, `target` 은 대상 병해충/목적, `dose` 는 말한 그대로("1000배", "한 통"). when: applied(살포·투입함) / planned(할 예정) / recommended(컨설턴트가 권고) / unknown. `date_hint` 는 통화일이 아닌 시점 표현 원문("지난주", "그저께"); 통화일이거나 시점 언급이 없으면 null.
