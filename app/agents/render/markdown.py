...
H1 제목은 없다(2026-09-10 제거 — 앱에 제목 텍스트가 그대로 노출됐다). 첫 줄은 `> 📝 통화 요약 / > 💬 격려` 인용 블록으로
섹션이 아니다(앱 블록에 대응하지 않고 prefill 에도 들어가지 않는다). 메타 표(작성일자·작물·생육단계·정식일·통화)는
맨 뒤 `## 근거 발화` 와 `## 참고` 사이(public 은 마지막 섹션 뒤·푸터 앞)에 둔다.
