...
**작물 고정 모드**(`ctx.hints.crop_fixed`, 2026-09-16 — 날짜별 일지 `POST /v1/daily-diaries` 의 `crop` 이 있을 때만
`daily_job.build_daily_context` 가 켠다; 호출자 hints 로는 못 켬). 위 1~6 을 타지 않고 `choose_fixed_target` 이 대상 **1건**만 만든다:

1. `hints.prdlst_code` 가 농가 등록 목록의 코드와 일치 → 그 작물(이름은 등록 이름으로 정규화)
2. 아니면 `hints.prdlst_nm` 을 등록 목록에 `_match_crop` → 일치하면 그 작물(코드 채움)
3. 아니면 표준 품목 전체(`_standard_prdlsts`; 코드가 없고 등록 목록에서도 못 찾을 때만 조회)에서 코드 일치 → 이름 정확 일치
4. 그래도 없으면 요청값 그대로. 등록 목록이 있는데 거기 없으면 `registered=False` + 경고
   `"{작물}: 농가 등록 작물에 없음 — 고정 작물로 일지 생성(미등록 작물)"`. `reason="fixed"`, `resolved=True` 항상(코드가 없어도 UNRESOLVED 가 아님).

사실 배분(`route_facts_fixed`): `crop` 이 있으면 [고정 작물 + 등록 목록의 나머지 + 통화 언급 작물] 에 `_match_crop` — 고정
작물에 맞거나 어디에도 안 맞으면(잡음) 포함, **다른 작물로 확인되면 제외**하고 작물별 건수를 경고
`"{작물} 관련 항목 {n}건 제외(작물 고정: {고정 작물})"` 로 남긴다. `crop` 이 **없는** 사실은 원칙적으로 고정 작물로 가되,
**근거 발화에 다른 작물명만 등장하면 제외**한다(`_other_crop_in_evidence` — 고정 작물명도 같이 나오면 보류·포함). 이 규칙이
필요한 이유: extract 프롬프트는 `crop` 을 "농가 작물 목록의 이름 또는 null" 로만 채우므로 목록 밖 작물(토큰 없이 hints 로만
만든 목록이면 고정 작물뿐)의 사실은 전부 `crop=None` 으로 온다 — 2026-09-16 dev 스모크에서 딸기 고정 일지에 파프리카 진딧물이
섞인 것이 이 경로였다. hints 로 만든 목록의 고정 작물은 표준 품목에서 빠진 쪽을 보완한다 — 코드 없으면 이름으로 코드를,
**코드만 받았으면 코드로 이름을**(이름 자리에 코드가 들어오면 진짜 작물의 사실까지 '타작물' 로 빠져 EMPTY 가 된다 — dev 실측).
등록 목록에서 코드·이름이 다 확정되면 표준 목록을 안 가져온다. `PipelineEmpty` 로 EMPTY 가 돼도 제외 경고는 `generation.warnings` 에 남는다. matcher 는 exact > substring 이라 등록 목록에
`토마토`·`방울토마토` 가 둘 다 있고 `방울토마토` 로 고정했으면 사실 `토마토` 는 제외되고, `토마토` 가 등록에 없으면 substring
으로 포함된다. 후속·조치는 그대로. "대표 작물에 배정" 경고는 나오지 않는다.
...
**보는 것**: 작물명, 일지 날짜, **렌더된 마크다운**(H1 아래 `> 📝/💬` 요약·격려 블록은 `strip_lead_quotes` 로 뗀 것 —
