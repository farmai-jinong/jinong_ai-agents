...
사실 배분(`route_facts_fixed`): `crop` 이 없는 사실은 고정 작물로. `crop` 이 있으면 [고정 작물 + 등록 목록의 나머지 + 통화 언급
작물] 에 `_match_crop` — 고정 작물에 맞거나 어디에도 안 맞으면(잡음) 포함, **다른 작물로 확인되면 제외**하고 작물별 건수를
경고 `"{작물} 관련 항목 {n}건 제외(작물 고정: {고정 작물})"` 로 남긴다. matcher 는 exact > substring 이라 등록 목록에
