...
| GET | `/v1/upstream/health` | STT / LLM(openai·jinong: `/models`, gemini: Vertex publisher model 조회) / S3(head_bucket) / farmos 도달성 + `pipeline` + **`config`**(비밀 아닌 유효 설정: `api_markdown_view, s3_prefix, storage_impl, public_base_url, callback_enabled, summary_callback_set, call_agent_callback_enabled, callback_include_artifact_keys, term_fix_enabled, verify_diary_enabled` — 배포 스모크가 환경 드리프트 판정에 씀). `AP_BACKEND_BASE_URL` 이 설정돼 있으면 `ap_backend` 항목도 포함 |
