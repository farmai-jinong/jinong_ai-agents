...
강등 시 같은 `diary.md.j2` 를 `status=EMPTY` 로 재렌더(섹션 집합은 그대로, 내용만 `언급 없음`)하고 `prefill` 을 회수한다. 검수 LLM 입력에서는 H1 아래 `> 📝 통화 요약 / > 💬 격려` 인용 블록을 떼어낸다(`strip_lead_quotes`) — 통화 요약이 빈 일지를 '내용 있음'으로 보이게 하지 않기 위해. 판정은 `structured.verify` 에 남는다.
