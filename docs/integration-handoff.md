...
```json
{
  ...
  "result": {
    ...
    "diaries": [{
      ...
      "markdown": "> 📝 **통화 요약** · …\n> 💬 …\n\n## 주요 농작업\n…(총 1,800자 내외; H1 제목 없음, 메타 표는 근거 발화·참고 사이, `##` 섹션은 항상 주요 농작업·기타 기록사항·병해충·방제이력·농작업 사진·투입 제품·향후 작업·확인 계획·근거 발화·참고)",
      "structured": {
        "prdlst_code": null, "prdlst_nm": "마늘", "diary_date": "2026-08-21", "status": "PARTIAL",
        "schema_version": "1",
...
- `diaries[]`는 통화에서 다룬 **작물별** 1건씩. `prdlst_code`는 farmos 품목코드이며, 작물을 확정하지
  ...
  **농가 등록 작물이 아닌 언급 작물**도 1건으로 온다(2026-09-14, 예: 파프리카 농가가 토마토 통화 → 토마토 일지): `prdlst_code` 는 표준 품목 이름이 정확히 일치할 때만, 별도 필드 없이 마크다운 메타 표 `작물` 행에 `(미등록 작물)` 표시
...
- **통화에 다른 작물 얘기가 섞여 있으면 그 부분은 뺍니다.** 예: 딸기로 고정했는데 파프리카 병해 얘기가 있으면 파프리카
  ...
  작물이 특정되지 않은 내용(관수·환기 등)과 후속·조치는 고정 작물 일지에 들어갑니다. (판정은 사실에 붙은 작물명과
  그 사실의 근거 발화에 등장한 작물명으로 합니다 — 근거 발화에 고정 작물과 다른 작물이 같이 나오면 포함합니다.)
